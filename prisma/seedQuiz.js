...
export const INITIAL_QUIZ_SEED = [
  // --- KELAS 12: LISTRIK STATIS & SEARAH ---
  {
    grade: 12,
  ...
  {
    grade: 12,
    topic: 'Hukum Pergeseran Wien (Bintang Panas)',
    difficulty: 'Mudah',
    question: 'Permukaan bintang biru Rigel memancarkan radiasi termal dengan temperatur $T = 12.000\\text{ K}$. Tentukan panjang gelombang pada intensitas pancaran maksimum ($\\lambda_{max}$) jika tetapan Wien $C = 2{,}9 \\times 10^{-3}\\,\\text{m}\\cdot\\text{K}$!',
    questionLatex: '\\lambda_{max} \\cdot T = C \\implies \\lambda_{max} = \\frac{C}{T}',
    formulaHint: '1\\,\\text{nm} = 10^{-9}\\,\\text{m}',
    options: [
      { text: '241.7 nm', latex: '241{,}7\\,\\text{nm}', isCorrect: true },
      { text: '483.3 nm', latex: '483{,}3\\,\\text{nm}', isCorrect: false },
      { text: '500.0 nm', latex: '500\\,\\text{nm}', isCorrect: false },
      { text: '120.5 nm', latex: '120{,}5\\,\\text{nm}', isCorrect: false }
    ],
    explanation: '$$\\lambda_{max} = \\frac{C}{T} = \\frac{2{,}9 \\times 10^{-3}}{12000} = 2{,}417 \\times 10^{-7}\\,\\text{m} = 241{,}7\\,\\text{nm}\\; \\text{(Spektrum Ultraviolet)}.$$'
  },
...
async function main() {
  console.log('Seeding initial Quiz Questions to Prisma Postgres...');
  ...
  console.log(`Successfully seeded ${INITIAL_QUIZ_SEED.length} quiz questions!`);
}
