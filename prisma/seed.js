...
async function main() {
  console.log('Seeding initial news to Prisma Postgres database...');
  const count = await prisma.article.count();
  if (count === 0) {
    for (const item of INITIAL_NEWS_SEED) {
      await prisma.article.create({ data: item });
    }
    console.log(`Successfully seeded ${INITIAL_NEWS_SEED.length} articles!`);
  } else {
